        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mt-6">
                <x-card title="Search & Manage" class="mb-6">
                    <div class="mb-4">
                        <form method="GET" action="{{ route('admin.positions.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                            <div class="flex-1">
                                ...
                                <td class="px-3 py-3.5 text-sm text-gray-700 dark:text-gray-300">
                                    @if($position->members_count > 0)
                                        <span class="inline-flex items-center gap-1 rounded-full bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 text-xs font-medium text-amber-700 dark:text-amber-400">
                                            {{ $position->members_count }} {{ Str::plural('member', $position->members_count) }}
                                        </span>
                                    @else
                                    ...
                                    <div class="inline-flex gap-2">
                                        ...
                                        @if($position->members_count > 0)
                                            <span
                                                ...
                                                title="{{ __('Cannot delete: :count :member(s) hold this position.', ['count' => $position->members_count, 'member' => Str::plural('member', $position->members_count)]) }}"
                                            >
                                                {{ __('Delete') }}
                                            </span>
                                        @else
                                            <form method="POST" action="{{ route('admin.positions.destroy', $position) }}">
                                                ...
                                                <button
                                                    ...
                                                    onclick="return confirm('{{ __('Are you sure you want to delete this position?') }}')"
                                                >
                                                    {{ __('Delete') }}
                                                </button>
                                            </form>
