    ...
    <body class="font-sans antialiased bg-gray-950 text-white">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="relative max-w-4xl mx-auto">
                    ...
                    @foreach ($steps as $index => $step)
                        <div id="step-{{ $index + 1 }}" class="relative flex flex-col lg:flex-row items-start gap-6 lg:gap-8 mb-12 lg:mb-16 last:mb-0 scroll-mt-28">
                            <!-- Mobile: number badge -->
        ...
        <div
            x-data="{
                ...
                hasStarted: false,
                get buttonLabel() {
                    if (this.currentStep >= this.totalSteps) return 'Back to Top';
                    if (!this.hasStarted) return 'Let\'s Start';
                    return 'Next Step';
                },
                get buttonIcon() {
                    if (this.currentStep >= this.totalSteps) return 'up';
                ...
                init() {
                    ...
                    if (firstStep) {
                        ...
                        revealObserver.observe(firstStep);
                    }
                    ...
                    const stepObserver = new IntersectionObserver((entries) => {
                        entries.forEach(entry => {
                            if (entry.isIntersecting) {
                                ...
                                if (!isNaN(num)) this.currentStep = num;
                            }
                        });
                    }, { threshold: 0.4 });
        ...
        >
            <div class="bg-gray-900/90 backdrop-blur-md border border-white/10 rounded-2xl shadow-2xl shadow-amber-500/10 px-4 py-3 flex items-center gap-3">
                <!-- Compact mode: Let's Start / Back to Top (no step dots or counter) -->
                <template x-if="!hasStarted || currentStep >= totalSteps">
                    <button
                        @click="handleClick()"
                        class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-gray-950 text-sm font-semibold transition-all hover:shadow-lg hover:shadow-amber-500/20 group"
                    >
                        <template x-if="buttonIcon === 'start'">
                        ...
                        <template x-if="buttonIcon === 'up'">
                            <svg class="size-4 transition-transform group-hover:-translate-y-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Full mode: Step navigation (dots + counter + Next Step) -->
                <template x-if="hasStarted && currentStep < totalSteps">
                    <div class="flex items-center gap-3">
                        <div class="flex items-center gap-1.5">
                            <template x-for="i in totalSteps" :key="i">
                                <span
                                    class="block w-1.5 h-1.5 rounded-full transition-all duration-300"
                                    :class="i <= currentStep ? 'bg-amber-500' : 'bg-white/20'"
                                ></span>
                            </template>
                        </div>

                        <span class="text-xs text-gray-400 font-medium tabular-nums" x-text="'Step ' + currentStep + ' of ' + totalSteps"></span>

                        <span class="w-px h-5 bg-white/10"></span>

                        <button
                            @click="handleClick()"
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-gray-950 text-xs font-semibold transition-all hover:shadow-lg hover:shadow-amber-500/20 group"
                        >
                            <span>Next Step</span>
                            <svg class="size-3.5 transition-transform group-hover:translate-x-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
